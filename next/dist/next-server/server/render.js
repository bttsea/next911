...
ServerRouter.events = (0, mitt_1.default)();
...
function render(renderElementToString, element, ampMode) {
    ...
    try {
        ...
        head = head_1.default.rewind() || (0, head_1.defaultHead)((0, amp_1.isInAmpMode)(ampMode));
    }
    return { html, head };
}
function renderDocument(Document, { dataManagerData, props, docProps, pathname, query, buildId, canonicalBase, assetPrefix, runtimeConfig, nextExport, autoExport, skeleton, dynamicImportsIds, dangerousAsPath, hasCssMode, err, dev, ampPath, ampState, inAmpMode, hybridAmp, staticMarkup, devFiles, files, dynamicImports, }) {
    return ('<!DOCTYPE html>' +
        (0, server_1.renderToStaticMarkup)(react_1.default.createElement(amp_context_1.AmpStateContext.Provider, { value: ampState },
...
async function renderToHTML(req, res, pathname, query, renderOpts) {
    ...
    try {
        ...
        if (isSpr) {
            const data = await unstable_getStaticProps({
                params: (0, is_dynamic_1.isDynamicRoute)(pathname) ? query : undefined,
            });
    ...
    if ((0, utils_1.isResSent)(res) && !isSpr)
        return null;
    const devFiles = buildManifest.devFiles;
    const files = [
        ...new Set([
            ...(0, get_page_files_1.getPageFiles)(buildManifest, pathname),
            ...(0, get_page_files_1.getPageFiles)(buildManifest, '/_app'),
        ]),
    ];
    ...
    const docProps = await (0, utils_1.loadGetInitialProps)(Document, Object.assign(Object.assign({}, ctx), { renderPage }));
    // the response might be finished on the getInitialProps call
    if ((0, utils_1.isResSent)(res) && !isSpr)
        return null;
    let dataManagerData = '[]';
    ...
    if (!docProps || typeof docProps.html !== 'string') {
        const message = `"${(0, utils_1.getDisplayName)(Document)}.getInitialProps()" should resolve to an object with a "html" prop set with a valid html string`;
        throw new Error(message);
    }
    ...
    const inAmpMode = (0, amp_1.isInAmpMode)(ampState);
    ...
    if (inAmpMode && html) {
        ...
        html = await (0, optimize_amp_1.default)(html);
