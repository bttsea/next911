...
class Server {
    constructor({ dir = '.', staticMarkup = false, quiet = false, conf = null, dev = false, } = {}) {
        this.dir = (0, path_1.resolve)(dir);
        this.quiet = quiet;
        const phase = this.currentPhase();
        this.nextConfig = (0, config_1.default)(phase, this.dir, conf);
        this.distDir = (0, path_1.join)(this.dir, this.nextConfig.distDir);
        this.publicDir = (0, path_1.join)(this.dir, constants_1.CLIENT_PUBLIC_FILES_PATH);
        this.pagesManifest = (0, path_1.join)(this.distDir, this.nextConfig.target === 'server'
        ...
        if (compress && this.nextConfig.target === 'server') {
            this.compression = (0, compression_1.default)();
        }
        ...
        (0, spr_cache_1.initializeSprCache)({
            dev,
            distDir: this.distDir,
            pagesDir: (0, path_1.join)(this.distDir, this._isLikeServerless
    ...
    handleRequest(req, res, parsedUrl) {
        ...
        if (typeof parsedUrl.query === 'string') {
            parsedUrl.query = (0, querystring_1.parse)(parsedUrl.query);
        }
        res.statusCode = 200;
    ...
    generateRoutes() {
        ...
        const routes = [
            {
                match: (0, router_1.route)('/_next/static/:path*'),
            ...
            {
                match: (0, router_1.route)('/_next/data/:path*'),
                fn: async (req, res, params, _parsedUrl) => {
                    ...
                    req.url = pathname;
                    const parsedUrl = (0, url_1.parse)(pathname, true);
                    await this.render(req, res, pathname, { _nextSprData: '1' }, parsedUrl);
                },
            },
            {
                match: (0, router_1.route)('/_next/:path*'),
            ...
            ...publicRoutes,
            {
                ...
                match: (0, router_1.route)('/static/:path*'),
                fn: async (req, res, params, parsedUrl) => {
            ...
            {
                match: (0, router_1.route)('/api/:path*'),
                fn: async (req, res, params, parsedUrl) => {
    ...
    async handleApiRequest(req, res, pathname) {
        ...
        await (0, api_utils_1.apiResolver)(req, res, params, resolverFunction ? require(resolverFunction) : undefined);
    }
    /**
    ...
    async resolveApiRequest(pathname) {
        return (0, require_1.getPagePath)(pathname, this.distDir, this._isLikeServerless, this.renderOpts.dev);
    }
    generatePublicRoutes() {
        const routes = [];
        const publicFiles = (0, recursive_readdir_sync_1.recursiveReadDirSync)(this.publicDir);
        const serverBuildPath = (0, path_1.join)(this.distDir, this._isLikeServerless ? constants_1.SERVERLESS_DIRECTORY : constants_1.SERVER_DIRECTORY);
        ...
        publicFiles.forEach(path => {
            ...
            if (!pagesManifest[unixPath]) {
                routes.push({
                    match: (0, router_1.route)(unixPath),
                    fn: async (req, res, _params, parsedUrl) => {
                        const p = (0, path_1.join)(this.publicDir, unixPath);
                        await this.serveStatic(req, res, p, parsedUrl);
                    },
                });
            }
    ...
    getDynamicRoutes() {
        ...
        return (0, utils_1.getSortedRoutes)(dynamicRoutedPages).map(page => ({
            page,
            match: (0, utils_1.getRouteMatcher)((0, utils_1.getRouteRegex)(page)),
        }));
    }
    ...
    async sendHTML(req, res, html) {
        ...
        return (0, send_html_1.sendHTML)(req, res, html, { generateEtags, poweredByHeader });
    }
    async render(req, res, pathname, query = {}, parsedUrl) {
        const url = req.url;
        if ((0, utils_3.isInternalUrl)(url)) {
            return this.handleRequest(req, res, parsedUrl);
        }
        if ((0, utils_3.isBlockedPage)(pathname)) {
            return this.render404(req, res, parsedUrl);
        }
    ...
    async findPageComponents(pathname, query = {}) {
        ...
        if (query.amp) {
            try {
                return await (0, load_components_1.loadComponents)(this.distDir, this.buildId, (pathname === '/' ? '/index' : pathname) + '.amp', serverless);
            }
            catch (err) {
                if (err.code !== 'ENOENT')
                    throw err;
        ...
        return await (0, load_components_1.loadComponents)(this.distDir, this.buildId, pathname, serverless);
    }
    ...
    async renderToHTMLWithComponents(req, res, pathname, query = {}, result, opts) {
        ...
        if (!isSpr) {
            ...
            return (0, render_1.renderToHTML)(req, res, pathname, query, Object.assign(Object.assign({}, result), opts));
        }
        ...
        const sprCacheKey = (0, url_1.parse)(req.url || '').pathname;
        // Complete the response with cached data if its present
        const cachedData = await (0, spr_cache_1.getSprCache)(sprCacheKey);
        if (cachedData) {
        ...
        if (isLikeServerless && isSprData) {
            const curUrl = (0, url_1.parse)(req.url || '', true);
            req.url = `/_next/data${curUrl.pathname}.json`;
        }
        const doRender = (0, coalesced_function_1.withCoalescedInvoke)(async function () {
            let sprData;
            let html;
            let sprRevalidate;
            let renderResult;
            ...
            if (isLikeServerless) {
                ...
                renderResult = await (0, render_1.renderToHTML)(req, res, pathname, query, renderOpts);
                html = renderResult;
        ...
        return doRender(sprCacheKey, []).then(async ({ isOrigin, value: { html, sprData, sprRevalidate } }) => {
            ...
            if (isOrigin) {
                await (0, spr_cache_1.setSprCache)(sprCacheKey, { html: html, pageData: sprData }, sprRevalidate);
            }
            return null;
        });
    }
    renderToHTML(req, res, pathname, query = {}, { amphtml, dataOnly, hasAmp, } = {}) {
        return this.findPageComponents(pathname, query)
            .then(result => {
            return this.renderToHTMLWithComponents(req, res, pathname, query, result, Object.assign(Object.assign({}, this.renderOpts), { amphtml, hasAmp, dataOnly }));
        }, err => {
    ...
    async render404(req, res, parsedUrl) {
        const url = req.url;
        const { pathname, query } = parsedUrl ? parsedUrl : (0, url_1.parse)(url, true);
        if (!pathname) {
            throw new Error('pathname is undefined');
        }
        res.statusCode = 404;
    ...
    async serveStatic(req, res, path, parsedUrl) {
        ...
        try {
            await (0, serve_static_1.serveStatic)(req, res, path);
        }
        catch (err) {
    ...
    isServeableUrl(path) {
        ...
        if (resolved.indexOf((0, path_1.join)(this.distDir) + path_1.sep) !== 0 &&
            resolved.indexOf((0, path_1.join)(this.dir, 'static') + path_1.sep) !== 0 &&
            resolved.indexOf((0, path_1.join)(this.dir, 'public') + path_1.sep) !== 0) {
    ...
    get _isLikeServerless() {
        return (0, config_1.isTargetLikeServerless)(this.nextConfig.target);
    }
}
